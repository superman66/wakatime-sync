module.exports = (function(e, t) {
  ...
  104: function(e, t, r) {
    ...
    const {
      ...
      GIST_ID: c,
      SCU_KEY: d,
      TELEGRAM_BOT_TOKEN: l,
      TELEGRAM_CHAT_ID: m
    } = process.env
    const g = 'https://wakatime.com/api/v1'
    const h = `${g}/users/current/summaries`
    const y = `https://sctapi.ftqq.com`
    const f = l && m ? `https://api.telegram.org/bot${l}/sendMessage` : null
    const b = new n(u)
    const _ = new o({ auth: `token ${p}` })
    ...
    function getTelegramMessageContent(e, t) {
      if (t.length > 0) {
        const { projects: r, grand_total: n, languages: i, categories: s, editors: o } = t[0]
        function mdBlock(e, t) {
          let r = `*${e}*\n`
          t.forEach(e => {
            r += `• *${e.name}*: ${e.text}\n`
          })
        ...
        return (
          `*Wakatime Report Breakdown of ${e}*\n*Total:* ${n.text}\n\n` +
          mdBlock('Projects', r) +
          '\n' +
          mdBlock('Languages', i) +
          '\n' +
          mdBlock('Editors', o) +
          '\n' +
          mdBlock('Categories', s)
        )
      }
    }
    function getMySummary(e) {
      return a.get(h, { params: { start: e, end: e, api_key: u } }).then(e => e.data)
    }
    async function updateGist(e, t) {
      const r = ''
      try {
        await _.gists.update({
          gist_id: c,
    ...
    async function sendMessageToWechat(e, t) {
      if (typeof d !== 'undefined') {
        return a.get(`${y}/${d}.send`, { params: { text: e, desp: t } }).then(e => e.data)
      }
    }
    async function sendMessageToTelegram(e, t) {
      if (l && m) {
        return a
          .post(f, { chat_id: m, text: `*${e}*\n${t}`, parse_mode: 'Markdown' })
          .then(e => e.data)
      }
    }
    const v = async e => {
      ...
      try {
        ...
        const n = `${t} update successfully!`
        const i = getMessageContent(t, r.data)
        const s = getTelegramMessageContent(t, r.data)
        await sendMessageToWechat(n, i)
        await sendMessageToTelegram(n, s)
      } catch (r) {
        if (e === 1) {
          console.error(`Unable to fetch wakatime summary\n ${r} `)
          await sendMessageToWechat(`[${t}]failed to update wakatime data!`)
          await sendMessageToTelegram(`[${t}]failed to update wakatime data!`, '')
          return
        }
  ...
  280: function(e, t) {
    ...
    Comparator.prototype.parse = function(e) {
      var t = this.options.loose ? o[F] : o[H]
      var r = e.match(t)
      if (!r) {
      ...
      return new Promise(function dispatchHttpRequest(t, r) {
        ...
        if (!A && A !== false) {
          ...
          if (L) {
            ...
            var I = true
            if ($) {
              var M = $.split(',').map(function trim(e) {
                return e.trim()
              })
              I = !M.some(function proxyMatch(e) {
                if (!e) {
                  return false
                }
                if (e === '*') {
        ...
        var B
        var F = x && (A ? y.test(A.protocol) : true)
        if (e.transport) {
          B = e.transport
        ...
        var H = B.request(R, function handleResponse(t) {
          if (H.aborted) return
          var r = t
          var s = t.req || H
